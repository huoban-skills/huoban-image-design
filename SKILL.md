...
| 手机端（上面任一页面的手机版、企业微信消息） | mobile | mobile；手机版工作台、详情页的内容取舍另读对应页面篇 | 一律 2～3 屏流程，每屏界面类型不同；不套产品壳，用手机宏 |
...
- 并排用 `<hb-row spans="16|8">`，矮组件（按钮组件、多项统计）别单独占一栏，用 `<hb-col>` 在同一栏叠两个；营销浮层用 `<hb-float w="…">`，固定在底图右下角，体内用 hb-row／hb-col 排一块小画面，体内放底层没有的 PC 组件（hb-list／hb-fields／hb-multistats），或只放一个 hb-phone 手机单屏，规则见 [references/canvas/marketing.md](references/canvas/marketing.md)。
...
出库审批.html：主管在企微里收到待办，点开核对明细后一键通过；皮肤晨光蓝；企业微信应用消息为仿写，未实测。
```
