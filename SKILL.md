...
- 按 [references/intake.md](references/intake.md) 分三条消息追问，每条发出即停，等用户答完再发下一条：
  1. 第 1 轮：画布、配色，加定不下来的清单维度（主角、讲哪些事、颗粒度）。
  2. 需求单初稿 ＋ 第 2 轮前 3 张图的细节题，同一条消息；初稿只让用户增删图，不问"确认吗"。
  3. 第 2 轮余下小轮（每轮 ≤3 张图）问完，发终稿需求单，问"按这份做吗"。
- 每轮的题一次发完（选择框工具装得下整轮才用，否则文字版一条消息）。
- 完成标准：第 2 轮每张图都问过、终稿需求单的细节列没有"待问"、用户对终稿回了确认。**初稿被认可不算确认；三条不齐，不进入步骤 2。**
