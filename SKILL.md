...
- 并排用 `<hb-row spans="16|8">`，矮组件（按钮组件、多项统计）别单独占一栏，用 `<hb-col>` 在同一栏叠两个；营销浮层用 `<hb-float w="…">`，固定在底图右下角，体内用 hb-row／hb-col 排一块小画面，体内放底层没有的 PC 组件（hb-list／hb-fields／hb-multistats），规则见 [references/canvas/marketing.md](references/canvas/marketing.md)。
