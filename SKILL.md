...
按 [references/intake.md](references/intake.md) 分三轮追问：第 1 轮画布类型、页面类型与端、角色、样式；第 2 轮按页面类型问那一支；第 3 轮问浮层、横幅与层次、数据文案。每轮用 intake 的选择题格式发出后**结束本轮回复，等用户答完再问下一轮**；第 1 轮的四项是决定不是事实，材料再详细也要问，用户在对话里明说过的才跳过。不把三轮压成一条，不跳过追问直接给需求单。三轮问完汇成需求单，**没拿到用户确认，不进入步骤 2。**
