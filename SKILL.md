...
- 并排用 `<hb-row spans="16|8">`，矮组件（按钮组件、多项统计）别单独占一栏，用 `<hb-col>` 在同一栏叠两个；营销浮层用 `<hb-float>`，放什么、怎么排见 [references/canvas/marketing.md](references/canvas/marketing.md) 第 3 节。
