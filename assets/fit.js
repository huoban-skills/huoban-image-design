...
(function () {
  function grow(s) {
    ...
    var floats = s.querySelectorAll('.float-anchor > .mk-float');
    if (!floats.length) return;
    var bottom = s.getBoundingClientRect().bottom, over = 0;
    floats.forEach(function (f) { over = Math.max(over, f.getBoundingClientRect().bottom - bottom); });
    if (over > 0) s.style.paddingBottom = Math.ceil(over + 24) + 'px';
  }
  ...
  function fit() {
    ...
    document.querySelectorAll('.stage').forEach(function (s) {
      s.style.zoom = 1;
      grow(s);                               // 挂在组件上的浮层是绝对定位，画布要留出它探出的高度
