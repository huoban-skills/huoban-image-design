...
def check(path, render=False, allow_local=False):
    ...
    _stage_txt = re.sub(r"<[^>]+>", " ", body)
    _verb = re.search(r"审批|审核|办理|处理|巡检|点检|保养|出库|入库|领用|领取|接单|派单|派工|报修|维修|确认|核对|登记|打卡|回访|签收|发货|收货|盘点|复核|催办|跟进|分配|回收|结算|核销|退回|驳回|报备|补录|派发|受理|验收|开单|录入", _stage_txt)
    if _verb:
        if "<table" in body and "op-btn" not in body:
    ...
    if re.search(r'data-kind="dashboard"', body):
        ...
        for n, pat in (("筛选", r'class="filter"'), ("趋势图（柱状／折线／面积／双轴，环图和条形图不算）", r'data-chart="(?:bar|line|area|biaxial)"'),
                       ("明细（透视表或表格列表）", r'class="til-|class="pivot|<table')):
            if not re.search(pat, main):
                sev = "High" if slide or n.startswith("趋势图") else "Medium"     # 默认尺寸下筛选、明细在宫格式里可无，降一档
