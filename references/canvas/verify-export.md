...
- `slide-height`（High）：演示尺寸的画面高超过 800，长宽比低于 1.5，放进 PPT 会被按高度缩小、字看不清；删一块组件或少几行。有浏览器按实测，没有按估算。
...
6 浮层底下是完整页面 通过
7 浮层没挡关键内容 改过：浮层宽度从 760 收到 640，露出明细表的状态列
8 信息密度够 通过
9 数据像真的 通过
...
手机图（kind=mobile）的手机壳只用 base.css 里收紧的投影，不要改回窗口级大投影：大投影超出画布会被导出切掉，透明 PNG 贴到有底色的幻灯片上会露出一块灰色矩形。导出后把 PNG 放到浅色底上看一眼，手机四周不应有方形灰边。
