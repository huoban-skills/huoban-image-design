...
```
首行表头，列名后可接类型 :tag（彩色选项）:tags（多值，值用 / 分）:user（人员，多人用 / 分）:ops（行内按钮，名:图标:颜色，多个用 / 分；颜色 blue／teal／green／orange／red／purple，不写为主色）；统计 :sum=值 :avg= :max= :min= :count=。
...
### hb-mtool
底部 56 栏：mode=list（列表工具栏，默认）/ obar（记录操作条，btns=快捷按钮 | … 排在条上方）/ app（应用页签栏）；体内可自定义项
