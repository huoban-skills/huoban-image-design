...
### hb-banner
横幅（富文本大标题预设）：第一行页面名称，第二行一句话介绍，无底色左对齐；card 出背景图卡片式（date、time、img）

```
横幅（富文本大标题预设）。第一行页面名称，第二行一句话介绍（口吻规则见 references/principles/workbench.md）。一律无底色、左对齐，标题直接坐在页底上；整块背景图用属性 card。
